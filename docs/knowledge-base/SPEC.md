...
202.x频道      → EPG认证获取RTSP单播地址 (通过115.233.40.x服务器)
                → 开放防火墙+路由让LAN设备可达
                → 生成三套m3u(标准/直通/兼容)适配不同播放器,详见 README
```
...
> ⚠️ **以下是 legacy `merge_m3u.py` 的行为,V2 没有实现名字黑名单。**
> 现行机制是**频道级** `channels.enabled=0`(不进 m3u、Dashboard 也不显示、ETL 不给它选主源,
> 库里记录全保留)。**没有"组级黑名单"这回事** —— 组只是 `channel_groups.group_name`。
> 实测(2026-07-25): 库里 `enabled=0` 的只有 `__UNKNOWN__`/`__JUNK__` 两个占位频道,
> 而下面名单里的 `好易购1高清`/`好享购` 从未被 V2 过滤。
>
> **购物台的现行处理**: 不屏蔽,单独归入 `购物` 分组(排在 `其他` 之后、`广播` 之前),
> 便于在播放器里整组跳过。`央广购物` 已挪入该组。

legacy 时期的名单(仅存档,勿当现行规则):
...
```
- (legacy)黑名单频道完全不输出到m3u,黑名单IP的源被过滤。**数据库(SQLite)不受影响,仍保留记录**。
- (legacy)**子串匹配**: `any(b in display for b in BLACKLIST_NAMES)`,所以可用前缀屏蔽一批。
