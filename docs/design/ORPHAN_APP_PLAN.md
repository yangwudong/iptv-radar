...
**现状：从不清理，只累积。** `orphan_export` 只 `makedirs(exist_ok=True)`，旧截图永远留着 ——
包括那些**已经被归并、不再是孤儿**的源的截图。NAS 上现在 51 张 / 8.8 MB。

用户场景：「第一周发现 8 个新源，第二周还有，第三周还有，我不是每周都有空弄」
→ 截图会无限堆积，页面里也会混着早已处理掉的源。

**策略：每次导出时重建"当前孤儿集合"的视图，只保留这一份。**

```
导出前:
  1. 计算本次孤儿源集合(channel_id IS NULL AND available=1)
  2. 删除 orphan-shots/ 下所有不属于本次集合的 jpg
     (含: 已归并的、已标 junk 的、已失效不再导出的)
  3. 同时清掉库里这些源的 sources.screenshots 字段(避免指向已删文件)
  4. 仍在集合内的源: 截图复用不重拍(本轮已实现)
  5. 覆盖写 orphans.json + orphan-review.html
```

这样无论你隔几周才弄，打开页面看到的**永远是当前真实待识别的那一批**，磁盘占用也稳定在
"当前孤儿数 × 3 张"（27 个源约 9 MB，不再增长）。

**已处理的不会重现**：`assign`/`new`/`junk`/`unknown` 落库后 `channel_id` 不再是 NULL，
自然从孤儿集合里消失；只有 `skip` 的会下次再出现（契约本意如此）。
...
| 3b | **只保留最近一份待识别包**（清理已不再是孤儿的截图 + 库字段） | 小（~25 行 + 测试） | **是**（已确认） |
...
| 待识别包保留策略 | ✅ **只保留最近一份**，每次导出重建当前孤儿视图并清理过期截图 |
