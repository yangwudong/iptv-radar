...
### rtp2httpd 把 RTSP 回看转成 HTTP(已实测,但有前提)

rtp2httpd 也能把运营商的 RTSP 单播/回看流代理成普通 HTTP:

```
http://<网关>:<端口>/rtsp/<RTSP服务器>:554/<path>?<原始query>&playseek=<起>-<止>
```

浙江电信实测结论:

- 鉴权 query(`accountinfo=`、`it=`)**原样透传**,不需要改写。
- `:554` 带与不带都行。
- 回看返回的是**真历史内容**(抓了4个不同时刻的画面,md5 全不相同)。
- **本地不需要时区 offset**。`playseek` 按运营商本地时间(北京时间)解释:
  请求 19:00,画面上的广播时钟显示 18:59:57 —— 早约3秒只是 GOP 对齐,
  不是 rtp2httpd 文档里警告的那个 8 小时偏差。若你的运营商真有偏差,用 `r2h-seek-offset`。
  *小技巧:* CCTV1 在 19:00 新闻联播前有一段赞助商报时广告,画面直接显示真实广播时间,
  是验证回看正确性的绝佳参照物。

在 NAS 网页播放器(飞牛影音)上实测发现的前提限制:

- 浏览器类播放器**普遍不实现 m3u 的 `catchup` 属性** —— URL 再正确也没有 UI 让用户选时间点。
- 本地 4K 频道是 **HEVC Main 10**,浏览器 / mpegts.js 解不了。

所以这条路适合**支持 catchup 的 HTTP 播放器**(如 Kodi 的 IPTV Simple Client),不适合纯浏览器播放器。
